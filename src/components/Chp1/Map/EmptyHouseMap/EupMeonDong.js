...
const EupMeonDong = ({ districts }) => {
  ...
  let EupMeonDongOption = [...new Set(EupMeonDongOptions.map((district) => district.properties.adm_nm.split(" ")[2]))].sort()
  ...
  useEffect(() => {
    if (searchResults.length){
      ...
      setEmptyPercentage((searchResults[0].properties.density*100).toFixed(2))
          ...
          <InfoRow>
            ...
            <TitleSection>
              <Title>우리 동네 빈집 지도</Title>
                <Paragraph>
                  <Line>전국에 광범위하게 늘어나고 있는 빈집. KBS취재진은 통계청 마이크로데이터를 바탕으로 ‘2019년 전국 읍면동 단위 빈집 지도’를 만들었습니다.</Line>
                </Paragraph>
                <GuideWrapper>
                  <GuideLine>
                ...
                <>
                  <ResultWrapper>
                    <EmptyHouseResult>
                      ...
                      <EmptyIndex idx={Number(emptyPercentage)}>빈집률&nbsp;
                        <AnimatedNumber 
